...
//        Files.writeString(MASTER.toPath(),commit.generateID());//跟新master，应该更新currentbranch而不是一味更新master
    ...
    public static void checkout(String[] args){
        ...
        if (args.length == 3 && args[1].equals("--")){  //checkout -- [file name]
            ...
            commitID = abbreviate(OBJECTS_DIR,commitID);
    ...
    public static String abbreviate(File FILE, String commitID){
    ...
    public static void merge(String branchname){
        initGitlet("merge");
        if (!fileContains(HEADS_DIR,branchname)){//给定分支不存在
            System.out.println("A branch with that name does not exist.");
            System.exit(0);
        }
        if (branchname.equals(curretnBranch())){//给定分支为当前分支
            System.out.println("Cannot merge a branch with itself.");
            System.exit(0);
        }
        Commit headcommit1 = headcommit();//获取headcommit
        Commit newcommit = readObject(join(OBJECTS_DIR,readContentsAsString(join(HEADS_DIR,branchname))), Commit.class);//获取newcommit
        Set<String> headSet= headcommit1.blobID.keySet(); //当前headcommit所有的追踪文件的set
        Set<String> newSet = newcommit.blobID.keySet(); //当前newcommit所有的追踪文件的set
        for (String newtrackfile:newSet){//但前commit未追踪，提取的追踪了，并且cwd中有该文件要覆盖
            if (!headSet.contains(newtrackfile) && fileContains(CWD,newtrackfile)){
                System.out.println("There is an untracked file in the way; delete it, or add and commit it first.");
...
//
            ...
            //合并后，合并的commit属于之前的分支，而不属于other
            File branchpointfile = join(HEADS_DIR,branchname);
            File headpointfile = join(HEADS_DIR,curretnBranch());
            ...
            try {
                Files.writeString(headpointfile.toPath(),mergecommit.commitID);//更新other存贮的ID信息
            } catch (IOException e) {
                e.printStackTrace(); // 打印异常信息
            }
            try {
    ...
    public static void conflict(String filename,Commit headcommit,Commit branchcommit,Commit mergecommit){
        ...
        File newblobfile = join(BLOB_DIR,filename);
        writeObject(newblobfile,newblob);
        mergecommit.blobID.put(filename,newblobfile);
    }
