...
public class ArrayHeap<T> implements ExtrinsicPQ<T>{
    ...
    private void swim(int index) {
        ...
        /** TODO: Your code here. */
        if (min(index, parentIndex(index)) != index || index == 1){//如果父节点优先级大（和spec相违背了？？？），那么子节点上去，父节点下来
            return;
        }else{
    ...
    private void sink(int index) {
        ...
        /** TODO: Your code here. */
        if (min(index, leftIndex(index)) == leftIndex(index) || min(index, rightIndex(index)) == rightIndex(index)){
            //如果满足sink条件，即任一个孩子大，则进入判断，继续选择出最小的孩子，进行交换；
            int miniIndex = min(leftIndex(index), rightIndex(index));
            swap(index, miniIndex);
            sink(miniIndex);
        }
    ...
    public void changePriority(T item, double priority) {
        ...
        for (i = 1; i <= size; i++) {
            if (item.equals( contents[i].item() ) ){
                initialPriority = contents[i].myPriority;
